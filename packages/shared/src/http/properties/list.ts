...
import { ensurePropertyStorageScaffold } from "@shared/properties/storage";
import {
  type OperationType,
...
} from "@shared/properties/mockStore";
...
const applyMappedFilters = (
  ...
  items
    ...
    .filter((item) => (filters.status ? item.status === filters.status : item.status !== "archived"))
    .filter((item) =>
      filters.projectId
