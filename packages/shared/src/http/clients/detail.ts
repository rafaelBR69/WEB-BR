...
import { hydrateDealRows, readDealsByClientId } from "@shared/deals/crud";
...
export const GET: APIRoute = async ({ params, url }) => {
  ...
  const mapped = mapClientRow(row, contact, { provider, agency, isProviderForProject: false });
  const dealRows = scopedOrgId ? await readDealsByClientId(client, scopedOrgId, id) : [];
  const deals = scopedOrgId ? await hydrateDealRows(client, scopedOrgId, dealRows) : [];
  const notificationsSummary =
    scopedOrgId
      ? await (async () => {
          try {
            const rows = await readNotificationRows(client, scopedOrgId, {
              includeClosed: true,
              clientId: id,
            });
            return buildNotificationEntitySummary(rows);
          } catch {
            return buildNotificationEntitySummary([]);
          }
        })()
...
export const PATCH: APIRoute = async ({ params, request }) => {
  ...
  const mapped = mapClientRow(updatedRow, updatedContact, {
    provider: nextProvider,
    agency: nextAgency,
    isProviderForProject: false,
  });
  const dealRows = await readDealsByClientId(client, effectiveOrgId, id);
  const deals = await hydrateDealRows(client, effectiveOrgId, dealRows);
  const refreshedNotificationsSummary = await (async () => {
    try {
      const rows = await readNotificationRows(client, effectiveOrgId, {
        includeClosed: true,
        clientId: id,
      });
      return buildNotificationEntitySummary(rows);
  ...
  return jsonResponse({
    ...
    data: {
      ...
      notifications_summary: refreshedNotificationsSummary,
    },
    meta: {
      storage: "supabase.crm.clients",
      persisted: true,
