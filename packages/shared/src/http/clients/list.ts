...
import {
  clientsMockFallbackDisabledResponse,
  isClientsMockFallbackEnabled,
} from "@shared/http/clients/mockFallback";
...
const fetchContactsByIds = async (
  ...
  const uniqueIds = uniqueUuids(contactIds);
  if (!client || !uniqueIds.length) return new Map<string, Record<string, unknown>>();
...
const fetchClientRelations = async (
  ...
  const uniqueClientIds = uniqueUuids(clientIds);
  if (!client || !uniqueClientIds.length) return empty;

  const providerRows: Array<Record<string, unknown>> = [];
  for (const chunk of chunkValues(uniqueClientIds, 150)) {
    ...
    if (error) throw new Error(`db_providers_read_error:${error.message}`);
    if (Array.isArray(data)) providerRows.push(...(data as Array<Record<string, unknown>>));
  }
  ...
  const agencyRows: Array<Record<string, unknown>> = [];
  for (const chunk of chunkValues(uniqueClientIds, 150)) {
    ...
    if (Array.isArray(data)) agencyRows.push(...(data as Array<Record<string, unknown>>));
  }
  ...
  let propertyLinks: Array<Record<string, unknown>> = [];
  {
    for (const chunk of chunkValues(uniqueClientIds, 150)) {
      ...
      if (Array.isArray(propertyLinkRows)) {
        propertyLinks.push(...(propertyLinkRows as Array<Record<string, unknown>>));
      }
    }
  ...
  if (projectId && providerIds.size > 0) {
    ...
    for (const chunk of chunkValues(Array.from(providerIds), 150)) {
  ...
  if (projectId) {
    ...
    if (linkedPropertyIds.length) {
      ...
      for (const chunk of chunkValues(linkedPropertyIds, 150)) {
...
export const GET: APIRoute = async ({ url }) => {
  ...
  if (!hasSupabaseServerClient()) {
    if (!isClientsMockFallbackEnabled()) {
      return clientsMockFallbackDisabledResponse(
        "clients_list_requires_supabase",
    ...
    return jsonResponse({
      ...
      meta: {
        ...
        storage: "mock_fallback_explicitly_enabled",
  ...
  const contactIds = uniqueUuids(rows.map((row) => asText(row.contact_id)));
  const clientIds = uniqueUuids(rows.map((row) => asText(row.id)));
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!hasSupabaseServerClient()) {
    if (!isClientsMockFallbackEnabled()) {
      return clientsMockFallbackDisabledResponse(
        "clients_create_requires_supabase",
        "Activa Supabase o habilita CRM_ENABLE_MOCK_FALLBACKS=true solo en desarrollo para usar mocks."
      );
    }
