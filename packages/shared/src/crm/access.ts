...
import { resolveCrmAuthFromCookies } from "@shared/crm/auth";
...
const ALL_MEMBERSHIP_ROLES = new Set<CrmMembershipRole>([
  "owner",
  "admin",
  "agent",
  "finance",
...
export const normalizeCrmPermissionList = (value: unknown): CrmPermission[] => {
  if (Array.isArray(value)) {
    ...
    value.forEach((entry) => {
      ...
      else if (typeof entry === "string" && isCrmPermission(entry.trim())) {
        unique.add(entry.trim() as CrmPermission);
      }
    });
...
export const resolveCrmOrgAccess = async (
  ...
  const activeMembership =
    (organizationIdHint ? memberships.find((item) => item.organization_id === organizationIdHint) : null) ??
    memberships[0];
  ...
  const permissions = resolveEffectiveCrmPermissions(
