import { asText, asUuid, asObject, asNumber } from "@/utils/crmPortal";
...
export const normalizeOperationInterest = (
  ...
  if (!value) return fallback;
  if (value === "rental") return "rent"; // Consistency with API
...
export const buildLeadRows = (
  ...
  return leads.map((lead) => {
    ...
    const projectCode = asText(projectRow?.legacy_code) ?? asText(projectPayload.project_legacy_code) ?? asText(mappedPayload.project_legacy_code);
    const propertyCode = asText(propertyRow?.legacy_code) ?? asText(projectPayload.property_legacy_code) ?? asText(mappedPayload.property_legacy_code);
