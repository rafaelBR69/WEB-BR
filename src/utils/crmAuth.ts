...
import { getSupabaseServerAuthClient } from "@/utils/supabaseServer";
...
export const setCrmAuthCookies = (cookies: AstroCookies, session: Session) => {
  ...
  const accessTtl = Number.isFinite(session.expires_in) && session.expires_in > 0 ? Math.floor(session.expires_in) : 3600;
  const refreshTtl = 60 * 60 * 24 * 30;
  const expiresAt = Number.isFinite(session.expires_at) && session.expires_at > 0 ? Math.floor(session.expires_at) : null;
...
export const resolveCrmAuthFromCookies = async (cookies: AstroCookies): Promise<CrmAuthResolution> => {
