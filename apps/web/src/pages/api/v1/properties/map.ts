...
export const GET: APIRoute = async ({ url }) => {
  try {
    const requestedLang = String(url.searchParams.get("lang") ?? DEFAULT_LANG)
      .trim()
      .toLowerCase();
    ...
    const missingCoordinatesCount = availableProperties.filter((property) => {
      ...
      const parent = property?.parent_id
        ? propertyById.get(String(property.parent_id))
        : null;
    ...
    const rawByListingType = availableProperties.reduce<Record<string, number>>(
      (acc, property) => {
    ...
    const diagnostics = {
      rows: {
        ...
        soldOrPrivate: allCards.filter(
          (card) => card.status === "sold" || card.status === "private"
        ).length,
    ...
    console.error("[properties-map] fatal", error);

    return new Response(
      JSON.stringify(
        {
          ok: false,
          error: "properties_map_failed",
