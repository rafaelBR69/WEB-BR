...
export const POST: APIRoute = async ({ cookies, params, request }) => {
  const clientId = asUuid(params.id);
  const body = (await parseJsonBody<Record<string, unknown>>(request)) ?? {};
  const organizationIdHint = asText(body.organization_id);

  if (!clientId) return jsonResponse({ ok: false, error: "invalid_client_id" }, { status: 400 });
  ...
  try {
    ...
    return jsonResponse(
      { ok: false, error: "client_deal_create_failed", details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
  }
};
